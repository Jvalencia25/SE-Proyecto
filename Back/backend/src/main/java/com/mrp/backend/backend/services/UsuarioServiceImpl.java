...
public class UsuarioServiceImpl implements UsuarioService{
    ...
    public Usuario save(Usuario user) {
        ...
        Optional<Usuario> u = repository.findByCorreo(user.getCorreo());

        if (u.isPresent()) throw new exception("Ya existe un usuario con el mismo email");

        String encoderPassword = this.passwordEncoder.encode(user.getContraseña());
